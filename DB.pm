...
sub Auth {
    ...
    # new version of bcrypt written by k1b3r
    elsif ($GetPw =~ m{^\$([\w\d]{2})\$([\d]{1,2})\$(.+?)$}xmsi) {
         $Kernel::OM->Get('Kernel::System::Log')->Log(
                Priority => 'debug',
                Message  => "PASSWORD BCRYPT PASS $1 $2 $3 $Pw",
         );

         if (bcrypt_check($Pw, $GetPw)) {
                $Kernel::OM->Get('Kernel::System::Log')->Log(
                         Priority => 'debug',
                         Message  => "PASSWORD BCRYPT MATCH",
                );
                $CryptedPw = $GetPw;
         }
